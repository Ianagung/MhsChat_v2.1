...
public class ChatMain {
    ...
    private boolean receiveAckFlag = false;
    private int nomorPesanAck = 0;
    ...
    public void sendChat(String sChat) {
        ...
        System.out.println("Chat counter = "+ChatState.chatCounter);
    }

    public boolean isReceiveChat() {
        boolean flag = receiveChatFlag;
        if (receiveChatFlag) {
            setReceiveChatFlag(false);
        }
        return flag;
    }
                ...
                if (packetType == DEF.TYPE_RX_MSG) {
                    switch (subType) {
                        case DEF.MSG_TYPE_CHAT:
                            ...
                            chatEntry = new ChatEntry(sSource, sText.getStext(), getDateTimeNow(), ChatType.INCOMING, ChatStatus.RECEIVE, sText.getTextNumber());
                            if (sText.getStext() == null) {
                                nullText = "Data = null , tidak tidak bisa ditampilkan";
                                chatEntry = new ChatEntry(sSource, nullText, getDateTimeNow(), ChatType.INCOMING, ChatStatus.RECEIVE, sText.getTextNumber());
                            }
                            int idx = destinationToIndex[source];
                            logsChat[idx].logChatContentList.add(chatEntry);
                            setReceiveChatFlag(true);
                            break;
        ...
        for (int i = 0; i < n_user; i++) {
            try {
                ...
                for (int j = 0; j < n_chat; j++) {
                    ...
                    System.out.println("nomor chat = "+nomor+" status = "+ status);
                }
            } catch (IOException ex) {
    ...
    /**
     * @return the receiveAckFlag
     */
    public boolean isReceiveAckFlag() {
        boolean flag = receiveAckFlag;
        if (receiveAckFlag) {
            receiveAckFlag = false;
        }
        return flag;
    }

    /**
     * @return the nomorPesanAck
     */
    public int getNomorPesanAck() {
        return nomorPesanAck;
    }

    /**
     * @param receiveChatFlag the receiveChatFlag to set
     */
    public void setReceiveChatFlag(boolean receiveChatFlag) {
        this.receiveChatFlag = receiveChatFlag;
    }
